...
export const registerTools = (server: McpServer, storageConfig: StorageConfig, serverConfig?: McpServerConfig) => {
  const tools = [
    identityTool(storageConfig),
    retrieveTool(storageConfig),
    uploadTool(storageConfig, serverConfig), // Pass server config for URL upload configuration
  ];
